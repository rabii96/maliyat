        ...
        <div class="page-content">
            ...
            <div class="row clearfix">
                <div class="col-md-12">
                    ...
                    <form id="addExpenseForm" action="{{ route('editExpense' , ['id' => $expense->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="portlet light ">
                        <div class="portlet-title">
                            <div class="caption font-dark">
                                <i class="icon-layers font-dark"></i>
                                <span class="caption-subject bold uppercase">تعديل مصروف</span>
                            </div>
                        ...
                        <div class="portlet-body">
                            ...
                            <div class="form-group">
                                ...
                                <input value="{{ $expense->name }}" autocomplete="off" type="text" name="name" id="name" class="form-control" placeholder=""> 
                            </div>
                            </div>     
                                ...
                                <select id="type_id" name="type_id" class="form-control select2 select-hide">
                                    ...
                                    @if($expenseTypes)
                                        @foreach($expenseTypes as $ex)
                                            <option value="{{ $ex->id }}" {{ $expense->expense_type_id == $ex->id ? 'selected' : '' }}>{{ $ex->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="details" class="form-control" rows="5">{{ $expense->details }}</textarea>
                            </div>
                            </div>   
                            ...
                            <div class="form-group">
                                ...
                                <select id="project_service_id" name="project_service_id" class="form-control select2 select-hide">
                                    ...
                                    @foreach (@$projects as $p)
                                        <option data-type="project" value="{{ $p->id }}" {{ $type=='project' && $expense->project_id == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                                    @endforeach
                                    @foreach (@$services as $s)
                                        <option data-type="service" value="{{ $s->id }}" {{ $type=='service' && $expense->service_id == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            </div>   
                            <input type="hidden" value="{{ $type }}" name="type" id="type">
                            <input type="hidden" value="{{ $expense->project_id }}" name="project_id" id="project_id">
                            <input type="hidden" value="{{ $expense->service_id }}" name="service_id" id="service_id">
                                ...
                                <select id="employee_id" name="employee_id" class="form-control select2 ">
                                    ...
                                    @if($employees)
                                        @foreach($employees as $e)
                                            <option value="{{ $e->id }}" {{ $e->id == $expense->employee_id ? 'selected' : '' }}>{{ $e->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                                @if($employees)
                    ...
                    @endif        
                            ...
                            <div class="form-group">
                                ...
                                <select id="bank_id" name="bank_id" class="form-control select2 select-hide">
                                    ...
                                    @if($banks)
                                        @foreach($banks as $b)
                                            <option value="{{ $b->id }}" {{ $b->id == $expense->bank_id ? 'selected' : '' }}>{{ $b->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                                ...
                                <select id="transfer_method_id" name="transfer_method_id" class="form-control select2 select-hide">
                                    ...
                                    @if($transferMethods)
                                        @foreach($transferMethods as $t)
                                            @if($t->id != 0)
                                                <option value="{{ $t->id }}" {{$t->id == $expense->transfer_method_id ? 'selected' : '' }}>{{ $t->name }}</option>
                                            @endif
                                        @endforeach
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-icon">
                                    ...
                                    <input value="{{ $expense->value }}" autocomplete="off" dir="ltr" style="text-align: right" type="text" id="value" name="value" class="form-control" placeholder=""> 
                                </div>
                            </div>
                            </div>      
                                ...
                                <select id="percentage_id" name="percentage_id" class="form-control select2 ">
                                    ...
                                    @if($percentages)
                                        @foreach($percentages as $p)
                                            <option data-value="{{ $p->value }}" value="{{ $p->id }}" {{ $p->id == $expense->percentage_id ? 'selected' : '' }} >{{ $p->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-icon">
                                    <i class="fa fa-money font-green "></i>
                                    <input value="{{ $expense->value_plus_percentage }}" autocomplete="off" name="value_plus_percentage" id="value_plus_percentage" type="text" class="form-control" placeholder="" readonly> 
                                </div>
                            </div>
                            </div>  
                            ...
                            <div class="form-group">
                                ...
                                <input value="{{ $expense->date->format('m/d/Y') }}" autocomplete="off" name="date" id="date" type="text" class="form-control date" placeholder="">
                                </div>
                            </div>
                            </div> 
                            ...
                            <div class="form-group">
                                <label>ملاحظات</label>
                                <textarea name="remakrs" class="form-control" rows="5">{{ $expense->remarks }}</textarea>
                            </div>
                            </div> 
